...
class ERestEventListenerRegistry
{
	...
	public final function run()
	{
		...
		$onRest(ERestEvent::REQ_AUTH_TYPE, function($application_id) {
			if(isset($_SERVER['HTTP_X'.$application_id.'CORS']) || (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'OPTIONS')) {
				return ERestEventListenerRegistry::REQ_TYPE_CORS;
			} else if(isset($_SERVER['HTTP_X'.$application_id.'USERNAME']) && isset($_SERVER['HTTP_X'.$application_id.'PASSWORD'])) {
		...
		$onRest(ERestEvent::REQ_AUTH_USER, function($application_id, $username, $password) {
			if(!isset($_SERVER['HTTP_X'.$application_id.'USERNAME']) || !isset($_SERVER['HTTP_X'.$application_id.'PASSWORD'])) {
				return false;
			}
			if( $username != $_SERVER['HTTP_X'.$application_id.'USERNAME'] ) {
				return false;
			}
			if( $password != $_SERVER['HTTP_X'.$application_id.'PASSWORD'] ) {
				return false;
			}
			return true;
		});
